...
export async function generateMetadata({
  ...
  return {
    title: `${item.name} | ${item.category} | High Coastal Cannabis Mississauga`,
    description: itemData.metaDescription,
    alternates: {
      canonical: `https://www.highcoastalcannabis.com/item/${slug}`,
    },
    openGraph: {
      title: `${item.name} | High Coastal Cannabis`,
      description: itemData.metaDescription,
...
function getJsonLd(item: ItemProduct) {
  ...
  const offers: any = {
    "@type": "Offer",
    url: `https://www.highcoastalcannabis.com/item/${item.slug}`,
    priceCurrency: "CAD",
  ...
  return {
    ...
    "@type": "Product",
    name: item.name,
    image: item.image ? [item.image.startsWith('http') ? item.image : `https://www.highcoastalcannabis.com${item.image.startsWith('/') ? '' : '/'}${item.image}`] : undefined,
  ...
  return {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": item.category,
        "item": `https://www.highcoastalcannabis.com/items/${catSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": item.name,
        "item": `https://www.highcoastalcannabis.com/item/${item.slug}`
      }
    ]
  };
}
